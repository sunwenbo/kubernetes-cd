...
SERVER_NAME="ops-sre-kubernetes-cd"
CURRENT_VERSIONS="core_version_6_instances_3"
CLUSTER="core"
DEPLOY_TYPE="formal"
INSTANCE_NUMBER="1"
COMMIT_ID="8"
...
#deploymen
function Deployment () {
    curl -H "Content-type: application/json" -H "Authorization: ${TOKEN}" -X POST -d '{"service_name":"'${SERVER_NAME}'","commit_id":"'${COMMIT_ID}'","cluster":"'${CLUSTER}'","deploy_type":"'${DEPLOY_TYPE}'","instance_number":"'${INSTANCE_NUMBER}'","current_version":"'${OLD_VERSION}'"}' http://127.0.0.1:8000/api/cd/deployment|tee /tmp/result.txt
    ...
    while true
    do
        curl -s -o /tmp/result.txt -H "Content-type: application/json" "http://127.0.0.1:8000/api/cd/deployment?cluster=${CLUSTER}&commit_id=${COMMIT_ID}&deploy_type=${DEPLOY_TYPE}&service_name=${SERVER_NAME}"
